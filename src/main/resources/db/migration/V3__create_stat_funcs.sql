-- "CREATE OR REPLACE FUNCTION" can cause accidental definitions of overloaded
-- functions (which are confusing) so always explicitly DROP/CREATE.

-- Adapted from https://blog.faraday.io/how-to-do-histograms-in-postgresql/
...
RETURNS TABLE(bucket integer, bucket_range int4range, freq bigint, bar text)
AS $func$
BEGIN
...
RETURNS TABLE(bucket integer, num_comments int4range, freq bigint, bar text)
AS $func$
SELECT * FROM discrete_histogram(
    $$
        (
            SELECT
...
RETURNS TABLE(bucket integer, num_hours int4range, freq bigint, bar text)
AS $func$
SELECT * FROM discrete_histogram(
    format(
        $$
...
RETURNS TABLE(bucket integer, ttfc_hours int4range, freq bigint, bar text)
AS $func$
SELECT * FROM
discrete_histogram(
    format(
...
RETURNS TABLE(bucket integer, ttlc_hours int4range, freq bigint, bar text)
AS $func$
SELECT * FROM
discrete_histogram(
    format(
